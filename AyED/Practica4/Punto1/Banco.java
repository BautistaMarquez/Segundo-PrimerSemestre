...
public class Banco {
	...
	public static void procesarMovimientosOptimizado(Integer[] cuentas, Consulta[] consultas) {
		...
		for(int i=0; i < consultas.length; i++) {
			c = consultas[i];
			aux[c.getDesde()]  += c.getValor();	//entiendo que guarda todas las sumas que se van a hacer a esta pos, lo cual lo vuelve mucho mas eficiente que el anterior, el tema es que solo lo hace a la pos desde y no a todo el rango. 
			aux[c.getHasta()+1]-= c.getValor();
		}
		
		for(int i=0; i < cuentas.length; i++) {
			if(i > 0) {
			  aux[i] += aux[i-1];//no entiendo que haria aca, ya entiendo.Lo va modificando ya que solo guarda la pos en el que arranca a sumar y en el que termina entonces va modificano las pos en las que no puso nada con el valor anterior. Tiene sentido.
			}
